...
def preprocess_test_with_context(sample, tokenizer, clip_processor, 
                                 docsearch, max_num_images, max_tokens=256):
    """
    Preprocess the test data to include context retrieval using RAG.
    """
    info = json.loads(sample[0])
    
    # Retrieve context based on the text query
    query = info.get("text_list", "")
    context = retrieve_context_and_combine(query, docsearch)
    
    # Update the info with the combined text
    info["text_list"] = context
    
    # Continue with the existing preprocessing steps for test data
    sentences = info["text_list"]
    valid_images = [Image.open(io.BytesIO(base64.b64decode(img["image_base64"]))).convert("RGB") for img in info["image_info"]]

    # Preprocess and pad images
    images_tensors = preprocess_image(valid_images, clip_processor)
    ...
    tokenizer.padding_side = "right"
    text_tensor = tokenizer(text, max_length=max_tokens, truncation=True, padding="max_length", return_tensors="pt")

    return (images_tensors, (text_tensor["input_ids"], text_tensor["attention_mask"]))
# End RAGADDITION

#  New addition: RAGADDITION
def get_test_data_with_context(args, image_processor, tokenizer, docsearch):
    """
    Interface for getting the test data with context retrieval using RAG.
    """
    preprocess_fn = functools.partial(
        preprocess_test_with_context,
        clip_processor=image_processor,
        tokenizer=tokenizer,
        docsearch=docsearch,
    )
    ...
    dataset = wds.WebDataset(test_shards).map(preprocess_fn).batched(args.batch_size_test, partial=False)
